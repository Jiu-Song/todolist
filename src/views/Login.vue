...
// 1. 定义表单数据：username, password 两个 ref，初始值空字符串
const username=ref('')
const password=ref('')
...
//    - try: await authStore.login(username.value, password.value)
//    - 成功后 router.push('/')
//    - catch: errorMsg.value = 错误信息（如"登录失败，请重试"）
...
// ==================== 提示 ====================
// authStore.login() 内部已经做了：
//   - 调 API
//   - 存 localStorage
//   - 更新 Pinia state
//   - 跳转首页
// 所以这里只需要 try-catch 处理失败情况即可
